...
export default function JobsPage() {
  ...
  const [results, setResults] = useState<
    Record<string, ApplyResult | "error" | "applying">
  >({});
  ...
  async function handleApply(jobId: string) {
    if (!selectedResume) {
      setError("Upload a resume first before applying.");
      return;
    }
      ...
      <div className="max-w-2xl mx-auto p-8">
        ...
        {resumes.length === 0 && (
          <p className="text-sm text-amber-600 mb-6">
            Upload a resume from your dashboard before applying.
          </p>
        )}
              ...
              <div key={job.id} className="bg-white p-5 rounded border">
                ...
                {!result && (
                  <button
                    ...
                    className="mt-3 bg-slate-900 text-white text-sm px-4 py-1.5 rounded"
                  >
                    Apply
                  </button>
                )}
                {result === "applying" && (
                  <p className="text-sm text-slate-400 mt-3">Applying...</p>
                )}
                {result === "error" && (
                ...
                {result && typeof result === "object" && (
                  <div className="mt-3 bg-slate-50 p-3 rounded">
                    ...
                    <a
                      href={`/candidate/interview/${result.id}`}
